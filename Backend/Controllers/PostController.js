...
export const CreatePost = async (req, res) => {
  ...
  try {
    if (req.files.length > 0) {
      ...
      filelist.map((i) => img.push(i.filename));
...
export const findPost = async (req, res) => {
  try {
    ...
    const post = await Post.find(params).limit(limit).skip(start);
...
export const GetAllPost = async (req, res) => {
  try {
    ...
    const post = await Post.find().limit(limit).skip(start);
...
export const SimilarPosts = async (req, res) => {
  try {
    const size = 3; /*parseInt(req.query.size) */

    const post = await Post.find({ type: req.params.type }).aggregate([
      { $sample: { size: size } },
    ]);
